#This @rowindex variable will be used to order num_replies values and find the median.
...
#This query defines logic to find all relevant "num_replies" dimensions.
    
SELECT
  ROUND(AVG(prep.num_replies),2) AS avg_num_replies,
  (
	#This subquery defines the logic needed to find "median_num_replies."
  	  SELECT
  	  ROUND(AVG(num_replies_med_prep.num_replies),2)
  	  FROM
  	  (
  	  #This subquery orders num_replies from smallest to largest, assigns a rowindex inreasing by 1 to each row.
  		  SELECT
  		  @rowindex := @rowindex + 1 AS rowindex,
  		  prep.num_replies
  		  FROM prep
  		  ORDER BY num_replies
  	  ) as num_replies_med_prep
  	  WHERE
  	 num_replies_med_prep.rowindex IN (FLOOR(@rowindex / 2) , CEIL(@rowindex / 2)) #This logic is finding the median using the rowindex.
  ) AS median_num_replies,
  MIN(prep.num_replies) AS min_num_replies,
  MAX(prep.num_replies) AS max_num_replies,
